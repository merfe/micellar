...
class ModelController extends AdminController
{
	...
	public function actionList($name)
	{
        ...
        $pagination->setPageSize(max($pagination->limit,30));
        $search->setPagination($pagination);

        $page = isset($_GET[$name.'_page']) ? $_GET[$name.'_page'] : 1;
        Yii::app()->user->setState('adminPage',$page);
        ...
        if(isset($model->orderField)){
            $data=array_merge_recursive($data,array(
                'rowCssClassExpression'=>'"items[]_{$data->getPrimaryKey()}"',
	...
	public function actionCreate($name)
	{
		...
		if (Yii::app()->request->isPostRequest && isset($_POST[$name])) {
			$paths=array();
            $oldAttributes = $model->attributes; //my fix
			...
			foreach ($model->tableSchema->columns as $column) {
				...
				if ($widget=='file' || $widget=='image' || $widget=='customImage') {
					...
					if ($file instanceof CUploadedFile) {
                        ...
                        if($model->$attribute === 'null'){
                            ...
                            $model->$attribute=$oldAttributes[$attribute];
                        }
                    }
				}
			}
			...
			if ($model->save()) {
                ...
                foreach ($model->tableSchema->columns as $column) {
                    $attribute=$column->name;
                    $widget=$this->module->getAttributeWidget($model,$attribute);
                    if ($widget=='file' || $widget=='image' || $widget=='customImage') { // file or image
                        $model->$attribute = $oldAttributes[$attribute];
                    }
                }
            }
	...
	protected function redirectUser($name,$pk)
	{
		if (isset($_POST['_save'])) {
			$this->redirect($this->createUrl('model/list',array('name'=>$name, $name.'_page' => Yii::app()->user->getState('adminPage'))));
