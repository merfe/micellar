...
class YcmModule extends CWebModule
{
	...
	public function getModelsList()
	{
		...
		return array_unique($this->_modelsList, SORT_REGULAR);
	}


    protected function addMenu($name)
    {
        $this->_modelsList[$name]=array();
    }
	/**
	 ...
	 */
	protected function addModel($model, $index)
	{
		$model=(string)$model;
		if (!in_array($model,$this->excludeModels)) {
			$this->_modelsList[$index][]=$model;
		}
	}
	...
	public function createWidget($form,$model,$attribute)
	{
		...
		switch ($widget) {
			...
			case 'wysiwyg':
				...
				$this->controller->widget($this->name.'.extensions.imperavi-redactor-widget.ImperaviRedactorWidget',$options);
				echo $form->error($model,$attribute);
				if ($form->type==TbActiveForm::TYPE_HORIZONTAL) {
					echo '</div></div>';
				}
            ...
            case 'chosenMultipleOrder': //todo implement many selectors, format, script customization
                $options=array(
                    'data-placeholder'=>Yii::t('YcmModule.ycm',
                            'Choose {name}',
                            array('{name}'=>$model->getAttributeLabel($attribute))
                    ...
                    'multiple'=>'multiple',
                    'class'=>'span5 chosen-order-select',
                );
                $options=$this->getAttributeOptions($attribute,$options);
                $customOptions=$this->getAttributeOptions($attribute,$options,false,true);
                $this->controller->widget($this->name.'.extensions.chosenOrder.EChosenOrderWidget', array('selected'=>$customOptions['selected']));
                echo $form->dropDownListRow($model,$attribute,$this->getAttributeChoices($model,$attribute),$options);
                echo $form->hiddenField($model,$attribute,array('id'=>'chosenOrderHidden','value'=>'[]'));
                break;
            ...
            case 'customImage':
                ...
                if (!$model->isNewRecord && !empty($model->$attribute)) {
                    ...
                    ob_start();
                    echo '<p></p>';
                    echo CHtml::image($model->getFileUrl($attribute),Yii::t('YcmModule.ycm','Image'),array(
                            'class'=>'modal-custom-image',
                            'style' => 'max-width:200px'
                        )
                    );
                    echo '<p></p>';
                    Yii::app()->clientScript->registerScript('modal-image-delete', "
                    $('.modal-image-delete').on('click',
                    function(){
                    $(this).parent().children(':hidden').val('null');
                    $(this).parent().children('.modal-custom-image').remove();
                    $(this).remove();
                    return true;
                    }
                    );

                    $('.modal-image-delete-input').change(function (){
                    $(this).parent().children(':hidden').val('');
                    $(this).parent().children('.modal-custom-image').remove();
                    $(this).parent().children('.modal-image-delete').remove();
                    return true;
                    });
                    ");
                    echo CHtml::link(Yii::t('YcmModule.ycm','Delete'),'#',array(
                ...
                $options['class']='modal-image-delete-input';
                echo $form->fileFieldRow($model,$attribute,$options);
                break;
			case 'hide':
				break;
	...
	{
		return $this->uploadUrl.'/'.$file; //my fix
	}
	 ...
	 * @return array
	 */
	protected function getAttributeOptions($attribute,$options=array(),$recursive=false,$custom = false)
	{
		...
		if (isset($this->attributesWidgets->$optionsName)) {
            ...
            if(array_key_exists('custom',$attributeOptions)){
                $customData=$attributeOptions['custom'];
                unset($attributeOptions['custom']);
                if($custom){
                    return $customData;
                }
            }
