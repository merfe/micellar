            ...
            function touchCalculate( e ) {
                ...
                if ( ax >= config.moveX ) {
                    ...
                    if ( config.allowDiagonal && ay >= config.moveY ) {
                        if ( toright && tobottom ) {
                            triggerEvent( config.wipeDownRight, result, e.originalEvent );
                        }
                        else if ( toright && !tobottom ) {
                            triggerEvent( config.wipeUpRight, result, e.originalEvent );
                        }
                        else if ( !toright && tobottom ) {
                            triggerEvent( config.wipeDownLeft, result, e.originalEvent );
                        }
                        else {
                            triggerEvent( config.wipeUpLeft, result, e.originalEvent );
                        }
                    }
                    // Otherwise trigger horizontal events if X > Y.
                    else if ( ax >= ay ) {
                        if ( toright ) {
                            triggerEvent( config.wipeRight, result, e.originalEvent );
                        }
                        else {
                            triggerEvent( config.wipeLeft, result, e.originalEvent );
                        }
                    }
                }
                // If Y > X and no diagonal, trigger vertical events.
                    ...
                    if ( tobottom ) {
                        triggerEvent( config.wipeDown, result, e.originalEvent );
                    }
                    else {
                        triggerEvent( config.wipeUp, result, e.originalEvent );
                    }
                }
